...
${patient_name ? `Patient's Name: ${patient_name}` : ''}${
            gender
                ? `
Gender: ${gender}`
                : ''
...
Age: ${age}`
            ...
            level_of_responsiveness
                ? `
Patient is ${level_of_responsiveness}`
                : ''
        }${
            chief_complaint.label
                ? `
Chief Complaint: ${chief_complaint.label}`
                : ''
        }${
            notes
                ? `
...
https://www.google.com/maps/search/${address.replace(/\s/g, '+')}`
    ...
    remove_emergency_vehicle = (call_index, emergency_vehicle_index) => {
        this.active_calls[call_index] = {
            ...this.active_calls[call_index],
            emergency_vehicles: remove(
                emergency_vehicle_index,
                1,
                this.active_calls[call_index].emergency_vehicles
            )
        }
