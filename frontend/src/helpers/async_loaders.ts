...
const loader_function_matches_key = (function_name, args, key) => {
    ...
    const args_match = key_args
        .map((key_arg, i) => args[i] === undefined || key_arg === JSON.stringify(args[i]))
...
export const get_loader_for_class_instance = (class_instance, class_function, ...args) => {
    ...
    for (const loader_key of keys(class_instance._loaders)) {
...
export const setup_async_loaders = class_instance => {
    ...
    for (const prop_name of prop_names) {
        ...
        if (type(prop) === 'Function' || type(prop) === 'AsyncFunction') {
            ...
            const wrapped_function = (...args) => {
                ...
                const did_return_promise =
                    function_return != null && typeof function_return.then === 'function'
